...
# -*- coding: utf-8 -*-
"""
Script pour nettoyer les fichiers CLTART*.xlsx (Clients par Articles) et générer un Excel propre
Usage: python clean_cltart24.py [fichier_source.xlsx]
Si aucun fichier n'est spécifié, traite CLTART24.xlsx par défaut
"""
...
FICHIER_SOURCE_DEFAUT = "CLTART24.xlsx"
...
def extraire_donnees(fichier_source):
    """
    Extrait et nettoie les données du fichier Excel CLTART* (Clients par Articles).
    Structure réelle détectée :
    - Colonne A (0) : Code client (C00001, C00003...) ou Code article (241001KD, 40050010, etc.)
    - Colonne D (3) : Nom client ou Désignation article
    ...
    data_clean = []
    client_courant = None
    nom_client_courant = None
    
    for _, row in df.iterrows():
        col_a = row[0]   # Code client ou code article
        col_d = row[3]   # Nom client ou désignation article
        ...
        # Détection d'un code client (C00001, C00003, etc.)
        if val_a.upper().startswith("C") and len(val_a) >= 5 and val_a[1:].isdigit():
            client_courant = val_a
            # Le nom du client est dans la colonne D (3)
            if pd.notna(col_d) and isinstance(col_d, str):
                nom_client_courant = str(col_d).strip()
            else:
                nom_client_courant = val_a
            continue
        
        # Détection des lignes de détail (codes article)
        # Un code article peut être de différentes formes (241001KD, 40050010, 4KS01010, E004, etc.)
        # Ce n'est pas un code client (C00001...), donc c'est probablement un article
        code_article = None
        
        # Ignorer les codes qui sont clairement des clients
        if val_a.upper().startswith("C") and len(val_a) >= 5 and val_a[1:].isdigit():
            continue
        
        # Si ce n'est pas un code client, c'est un code article
        # Vérifier qu'il y a au moins une valeur numérique ou alphanumérique
        if len(val_a) > 0 and (val_a[0].isalnum() or val_a[0] in ['-', '_']):
            # Vérifier qu'il y a au moins du CA ou de la marge pour confirmer que c'est un article
            if pd.notna(col_l) or pd.notna(col_p):
                code_article = val_a
        ...
        # La désignation de l'article est dans la colonne D (3)
        designation = str(col_d).strip() if pd.notna(col_d) and isinstance(col_d, str) else ""
        
        data_clean.append({
            "Client": nom_client_courant or client_courant,
...
def main():
    print("=" * 70)
    print("NETTOYAGE CLTART*.xlsx (Clients par Articles)")
    print("=" * 70)
    ...
    if not os.path.exists(fichier_source):
        ...
        print(f"   Usage: python clean_cltart24.py [fichier_source.xlsx]")
        sys.exit(1)
