...
# -*- coding: utf-8 -*-
"""
Script pour nettoyer les fichiers STATARTART*.xlsx (Statistiques Articles) et générer un Excel propre
Usage: python clean_statartart.py [fichier_source.xlsx]
Si aucun fichier n'est spécifié, traite STATARTART.xlsx par défaut
"""
...
FICHIER_SOURCE_DEFAUT = "STATARTART.xlsx"
...
def extraire_donnees(fichier_source):
    """
    Extrait et nettoie les données du fichier Excel STATARTART* (Statistiques Articles).
    Structure réelle détectée :
    - Colonne A (0) : Code article (000109, 0001_NOV, 0104001, etc.)
    - Colonne D (4) : Désignation article
    ...
    for _, row in df.iterrows():
        ...
        # Un code article peut être de différentes formes (000109, 0001_NOV, 0104001, etc.)
        # Il doit avoir au moins un caractère alphanumérique
        if len(val_a) == 0:
            continue
        
        # Vérifier que c'est bien un code article (commence par un chiffre ou une lettre, pas un code famille F...)
        # Ignorer les codes famille qui commencent par F
        if val_a.upper().startswith("F") and len(val_a) >= 2 and val_a[1].isdigit():
            continue
        
        # Un code article doit avoir au moins un caractère alphanumérique
        if not val_a[0].isalnum() and val_a[0] not in ['-', '_']:
            continue
        ...
        # La désignation de l'article est dans la colonne D (4)
...
def creer_excel_clean(df_clean, fichier_sortie, nom_source):
    ...
    c_footer.value = f"{len(df_clean)} article(s)"
...
def main():
    print("=" * 70)
    print("NETTOYAGE STATARTART*.xlsx (Statistiques Articles)")
    print("=" * 70)
    ...
    if not os.path.exists(fichier_source):
        ...
        print(f"   Usage: python clean_statartart.py [fichier_source.xlsx]")
        sys.exit(1)
    ...
    try:
        ...
        print(f"  Articles de donnees : {len(df_clean)}")
